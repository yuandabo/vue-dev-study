...
// 参考资料 https://blog.csdn.net/qq2276031/article/details/106407647
...
// 通过 Key 和 vnode 的一些属性判断 vnode 是否相同 （只比较最外层
...
export function createPatchFunction (backend) {
  ...
  function updateChildren(
    ...
    let oldStartIdx = 0 // 设置老结点头指针索引为0
    let newStartIdx = 0 // 设置新结点头指针索引为0
    ...
    // 当老结点遍历完成 或者 新结点遍历完成时 结束循环
    while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
      if (isUndef(oldStartVnode)) {
        oldStartVnode = oldCh[++oldStartIdx] // Vnode has been moved left // 移动指针
        ...
        canMove &&
          nodeOps.insertBefore(parentElm, oldEndVnode.elm, oldStartVnode.elm) // 换位 将老结点末尾匹配的放入老结点后
        oldEndVnode = oldCh[--oldEndIdx]
        newStartVnode = newCh[++newStartIdx]
      } else {
        // 通过Keyp匹配
  ...
  function patchVnode(
    ...
    const oldCh = oldVnode.children // 取下一级结点
  ...
  return function patch(oldVnode, vnode, hydrating, removeOnly) {
    if (isUndef(vnode)) {
      // 新节点为空
      if (isDef(oldVnode)) invokeDestroyHook(oldVnode) // 老结点非空直接调用老节点的 destroy 生命周期函数
      return
    }
